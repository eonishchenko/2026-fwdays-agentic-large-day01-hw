...
Це монорепозиторій Excalidraw (`excalidraw-monorepo`) для браузерного редактора діаграм/дошки та пов'язаних npm-пакетів.

Основна ідея:
- `excalidraw-app` — основний вебзастосунок (UI для малювання, редагування, обміну сценами).
- `@excalidraw/excalidraw` — React-бібліотека для вбудовування Excalidraw в інші проєкти.
- Внутрішні пакети (`common`, `element`, `math`, `utils`) містять спільну доменну логіку.
- Колаборація та зберігання даних реалізуються через клієнтські інтеграції (Firebase + WebSocket).
...
- Vite 5
- SCSS/CSS
- Jotai (частково для стану UI)

### Інтеграції даних і realtime
- Firebase SDK (Firestore, Storage)
- socket.io-client (підключення до зовнішнього WS-сервера)

### Tooling і якість коду
- Yarn workspaces (монорепозиторій)
- ESLint + Prettier
- Husky + lint-staged
- Vitest (тести)
...
```text
/
├─ excalidraw-app/         # основний веб-клієнт
├─ packages/               # пакети монорепозиторію
│  ├─ excalidraw/          # публічний React SDK
│  ├─ element/             # моделі/операції елементів сцени
│  ├─ common/              # спільні типи/утиліти
│  ├─ math/                # математичні утиліти
│  └─ utils/               # допоміжні модулі
├─ examples/               # приклади інтеграції
├─ public/                 # статичні ресурси
├─ scripts/                # службові скрипти
├─ firebase-project/       # firebase rules/indexes
├─ docs/                   # документація
├─ Dockerfile              # збірка production image
├─ docker-compose.yml      # локальний запуск у контейнері
├─ package.json            # root скрипти/залежності
...
Явного окремого backend-сервісу в репозиторії не видно; клієнт підключається до зовнішнього WebSocket endpoint та Firebase.
